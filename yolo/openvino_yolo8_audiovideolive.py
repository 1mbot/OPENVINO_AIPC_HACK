...
def draw_detections_on_frame(frame, detections, labels, colors):
    ...
    # --- MODIFIED: Need to pass scaled coords for cropping ---
    scaled_boxes = [] 
    ...
    for label, score, box in detections:
        ...
        try:
            ...
            # --- NEW: Handle custom labels like 'Ameya' ---
...
def run_camera_detection(model_xml_path, face_rec_model_xml_path, device="AUTO"):
    ...
    # We'll run this on the CPU, as it's only a few inferences per second
    ...
    if not os.path.exists(normalized_face_rec_path):
        ...
        print("Run all_faces.py or check your path.")
        sys.exit(1)
    ...
    if not os.path.exists(EMBEDDINGS_FILE):
        print(f"Warning: Embeddings file not found at {EMBEDDINGS_FILE}. No faces will be recognized.")
        known_faces_db = {}
    else:
    ...
    while True:
        ...
        if img_np_for_inference is not None:
            ...
            if len(boxes) > 0:
                # --- MODIFIED: Create a copy for drawing ---
                # We need to process labels *before* drawing
                
                # We need to scale boxes first for cropping
                scale_x = frame_width / 640.0
                scale_y = frame_height / 640.0
                ...
                for i in range(len(class_ids)):
                    if class_ids[i] < len(labels):
                        ...
                        if class_ids[i] == HUMAN_FACE_CLASS_ID and len(known_faces_db) > 0:
                            ...
                            # Add some padding and ensure bounds
                            padding = 10
            ...
            if current_detected_labels != last_spoken_labels and (current_time - last_spoken_time) > speak_cooldown:
                if len(current_detected_labels) > 0:
                    spoken_text = "I see " + ", ".join(current_detected_labels)
                    speak(spoken_text)
            ...
            # Now draw all processed detections
            annotated_frame, _ = draw_detections_on_frame(frame, processed_detections, labels, colors)
